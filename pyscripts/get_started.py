...
def getStarted_step1(label, lineEdit_27, lineEdit_28):
    ...
    btn_Cancel = msg_box.button(QMessageBox.StandardButton.Cancel)
...
def getStarted_step2(label, lineEdit_27, lineEdit_28):
    ...
    msg_box.setStandardButtons(
        QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Open | QMessageBox.StandardButton.Save | QMessageBox.StandardButton.Cancel)
    btn_Ok = msg_box.button(QMessageBox.StandardButton.Ok)
    btn_Open = msg_box.button(QMessageBox.StandardButton.Open)
    btn_Save = msg_box.button(QMessageBox.StandardButton.Save)
    btn_Cancel = msg_box.button(QMessageBox.StandardButton.Cancel)
    if label.text().startswith('Ամսաթիվ'):
